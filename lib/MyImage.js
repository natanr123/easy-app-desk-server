...
export default class MyImage {
    ...
    static highRes(path, outputPath) {
        return  sharp(`./${path}`)
            .resize(512,512)
            .png()
            .toFile(outputPath);
    ...
    static resize(path, outputPath, width, height) {
        console.log('path: ', path, 'outputPath: ', outputPath, 'width: ', width, 'height: ', height);
        const temp = sharp(`./${path}`)
            .resize(width, height);
        return  temp
            .png()
            .toFile(outputPath);
